...
def predictioncorn():
    ...
    COUNT += 1
    if prediction[0] == 0:
        return render_template('Output.html', data=["Blight",'''....Xyway LFR Fungicide: An effective fungicide applied at planting to provide systemic protection against blight.
....Delaro 325 SC Fungicide: A broad-spectrum fungicide that controls multiple corn diseases including blight.
....Quilt Xcel Fungicide: Combines two active ingredients to protect against blight and other diseases, improving plant health.
....Stratego YLD Fungicide: Offers both preventive and curative activity against blight with long-lasting protection.
....Headline AMP Fungicide: Provides excellent control of corn blight with improved yield potential. '''])
    elif prediction[0] == 1:
        return render_template('Output.html', data=["Common_Rust", ''' Pyraclostrobin (Headline): Effective against common rust, providing protection through foliar application.
....Triazole (Tilt, PropiMax, Bumper): Broad-spectrum fungicides that control rust and other diseases.
....Strobilurin (Quadris, Quilt): Prevents spore germination and controls common rust.
....Tebuconazole (Folicur, Orius): Offers curative and protective action against common rust.
....Flutriafol (Topguard): Systemic fungicide providing protection against common rust and other fungal diseases.'''])
    elif prediction[0] == 2:
        return render_template('Output.html', data=["Gray_Leaf_Spot",''' Azoxystrobin (Quadris): Effective against gray leaf spot, offering preventive and curative action.
....Picoxystrobin (Aproach): Controls gray leaf spot and enhances plant health.
....Pyraclostrobin + Metconazole (Headline AMP): Combination fungicide providing broad-spectrum disease control including gray leaf spot.
....Propiconazole (Tilt): Systemic fungicide offering protection against gray leaf spot and other diseases.
....Benzovindiflupyr + Difenoconazole (Elatus): Provides long-lasting protection against gray leaf spot with systemic action.'''])
    else:
...
def predictionpotato():
    ...
    if prediction[0] == 0:
        ...
        return render_template('Output.html', data=["Potato_Early_blight",'''Mancozeb Flowable with Zinc Fungicide Concentrate ... 
                                                    Spectracide Immunox Multi-Purpose Fungicide Spray Concentrate For Gardens ...
                                                     Southern Ag – Liquid Copper Fungicide ... Bonide 811 Copper 4E Fungicide ... Daconil Fungicide Concentrate. ''' ])
    elif prediction[0] == 1:
        # cv2.imwrite('static/images/{}.jpg'.format(COUNT), img)

        return render_template('Output.html', data=["Potato_Late_blight", ''' 1. Ridomil Gold SL,
            2. Revus Top,
            3. Curzate 60DF,
            4. Gavel 75DF,
            5. Orondis Opti A + Orondis Opti B'''])
...
def predictiontomato():
    ...
    if prediction[0] == 0:
        ...
        return render_template('Output.html', data=["Leaf_Mold", [
            '1. Chlorothalonil-based fungicides',
            '2. Copper fungicides',
            '3. Improve air circulation in the greenhouse',
            '4. Reduce humidity levels',
            '5. Remove and destroy infected leaves'
        ]])
    elif prediction[0] == 4:
        ...
        return render_template('Output.html', data=["Spider_mites Two-spotted_spider_mite", [
            ...
            '2. Neem oil',
            '3. Miticides such as abamectin',
            '4. Predatory mites (biocontrol)',
            '5. Maintain proper irrigation to reduce plant stress'
        ]])
    elif prediction[0] == 6:
        return render_template('Output.html', data=["Target_Spot", [
            '1. Chlorothalonil-based fungicides',
            '2. Copper fungicides',
            '3. Ensure proper plant spacing to improve air circulation',
            '4. Remove and destroy infected plant debris',
        ...
        return render_template('Output.html', data=["Tomato_mosaic_virus", [
            '1. Use virus-free seeds and transplants',
            '2. Sanitize tools and equipment regularly',
            '3. Remove and destroy infected plants',
            '4. Avoid handling plants excessively',
            '5. Implement crop rotation'
        ]])
    else:
        return render_template('Output.html', data=["Healthy", 'Your plant is healthy. No treatment is needed.'])
